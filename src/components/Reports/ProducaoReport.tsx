...
import { FileText, User, Calendar, DollarSign, Building, Edit, Trash2, Plus, Filter } from 'lucide-react';
...
export const ProducaoReport: React.FC<ProducaoReportProps> = ({ onEdit, onDelete, onNew }) => {
  ...
  const filterData = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await dbHelpers.getProducaoMensalByMedico(parseInt(selectedMedico));
